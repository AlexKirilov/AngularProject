...
export class HandleErrorsService implements OnDestroy {
  ...
  constructor(public dialog: MatDialog, private services: ServiceProvider) {
    ...
    this.promptTest = {
      Title: '',
      ErrorMessage: 'Are you sure that you wish to delete those files?',
      Type: 'prompt'
    };
    this.tmp = this.error;
  ...
  openDialogSendClientAddressFormRequest(data: any, callback = null) {
    ...
    const dialogRef = this.dialog.open(this.component, {
      ...
      data: {
        ...
        msgType: 'clientAddressRequest',
        text: `Email request was send to the specified clients email address!`,
        sendReq: false,
  ...
  handleError(err: any): any {
    ...
    if (
      this.error &&
      this.error.error &&
      this.error.error.hasOwnProperty('sortColumn')
    ) {
      this.datashare.stopSpinnerContent();
      this.datashare.showSnackBar({
        message: 'The table can`t be sorted by this column name',
        action: ''
      });
      ...
      // Page or Data was/were not found
      ...
      this.error.ErrorMessage =
        'Sorry! We could not load your data. There was an Internal Server Error! Please contact with your administrator or try again later.';
      this.openDialog(this.error);
    } else if (
      err.status === 500 ||
      err.status === 503 ||
      ...
      // tslint:disable-next-line:max-line-length
      this.error.ErrorMessage =
        'We are sorry, but this service is unavailable for the moment! Please contact with your administrator or try again later.';
      this.openDialog(this.error);
    } else {
      this.openDialog(this.error);
