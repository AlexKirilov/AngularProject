...
export class SiteApiConnectionDataComponent implements OnInit, OnDestroy {
  ...
  public apiModels: string;

  private headerSiteID: string;
  private unscApiHeader: Unsubscribable;

  constructor(
    private datastore: DatastoreService,
  ) {
    this.unscApiHeader = this.datastore.getClientSiteID().subscribe(
      (id: any) => {
        this.apiHeader = `
            if (token) {
  ...
  ngOnDestroy(): void {
    if (this.unscApiHeader) { this.unscApiHeader.unsubscribe(); }
  }
